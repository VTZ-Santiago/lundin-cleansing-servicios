"""Generate COMPLEMENTARIA summary report: non-D positions by migration category and plant.

Sheet layout per segment Excel:
  Resumen              — pivot plant × category: rows, unique docs, value USD
  Por_Tipo_Posicion    — pivot (plant × category × position_type): rows, docs, value USD
  Apertura_Codigo      — pivot category × doc prefix + material presence
  VIGENTE              — detail rows, sorted by plant
  NO_VIG_SIN_SALDO     — detail rows; these DO NOT migrate
  SALDO_PEND_{YYYY}    — one sheet per delivery year for NO_VIGENTE_CON_SALDO (must migrate)
"""
...
def generate_complementaria_report(
    segment_id: str,
    label: str,
    df: pd.DataFrame,
    m01_config: dict,
    ...
    """Generate complementaria Excel report for one segment.

    Args:
        segment_id: e.g. "contratos"
        label: human-readable segment name
        df: full ruled dataframe, must contain migration_category
    ...
    output_path = output_dir / f"complementaria_{segment_id}_{ts}.xlsx"

    # Exclude TIPO_D rows — only report on COMPLEMENTARIA
